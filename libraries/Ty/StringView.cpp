...
ErrorOr<StringBuffer> StringView::to_buffer() const
{
    return TRY(StringBuffer::create_fill(*this));
}

ErrorOr<c_string> StringView::to_allocated_c_string() const
{
    auto buf = TRY(to_buffer());
    TRY(buf.write("\0"sv));
    TRY(buf.saturate());
    return buf.leak();
}
